...
const resourceKinds = new Set(['book','article','video','course','repo','doc','other'] as const);
...
export async function GET(req: Request) {
  ...
  if (!keyRaw) return NextResponse.json({ nodes: [], edges: [], items: [] }, { status: 200 });
  ...
  if (isUUID(keyRaw)) {
    ...
    const { data: t, error: terr } = await supabase
      .from('topics').select('id').eq('slug', keyRaw).maybeSingle();
  ...
  if (ids.length > 0) {
    ...
    const { data: res, error: resErr } = await supabase
      .from('resources')
      .select('id,title,type,difficulty,duration_min,cost_amount,cost_currency,url')
      .in('id', ids);
    ...
    if ((res?.length ?? 0) > 0) {
      ...
      for (let i = 0; i < sorted.length - 1; i++) {
        edges.push({ source: sorted[i].id, target: sorted[i + 1].id, weight: 1 });
      }
    }
  }
